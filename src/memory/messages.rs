...
mod tests {
    ...
    fn test_fts5_search() {
        ...
        assert!(results
            .iter()
            .any(|(_, content, _)| content.contains("async")));
        let results = search_memory(&conn, "pizza", 5);
        assert!(results
            .iter()
            .any(|(_, content, _)| content.contains("pizza") || content.contains("Pizza")));
        delete_memory(&sender);
    }
    ...
    fn test_persist_model_preference_round_trip() {
        ...
        assert_eq!(
            load_model_preference(&conn).as_deref(),
            Some("claude-3-haiku")
        );
        delete_memory(&sender);
    }
